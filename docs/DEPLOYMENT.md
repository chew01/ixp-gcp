...
2. Confirm the dashboard uses the correct datasource UID (`prometheus`). Check `monitoring/ixp-flows.json`.
3. Confirm the `ixp-flows-dashboard` ConfigMap is labelled: `kubectl get configmap ixp-flows-dashboard -n monitoring --show-labels`.
