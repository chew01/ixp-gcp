...
Prometheus is deployed via the **prometheus-community Helm chart**. The API gateway exposes metrics on `:9090` via a `ServiceMonitor` CRD. Grafana renders the dashboard defined in `monitoring/ixp-flows.json`.
...
| Target | Action |
|--------|--------|
| `make infra` | Minikube + Kafka (Strimzi) + Atomix + monitoring |
