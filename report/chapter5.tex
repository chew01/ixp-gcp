...
The instrumentation is implemented in a shared bootstrap layer and then consumed consistently by each service entry point. Instead of duplicating setup logic per service, the code uses \texttt{shared/otel/otel.go} and \texttt{shared/otel/otel-init.go} as the common initialization path.
...
Prometheus scrapes the collector and evaluates the alert rules defined for API availability, Atomix dependency health, latency congestion, and telemetry continuity. The rules are organized by service and failure semantics rather than by low-level metric origin.
...
\textbf{Replica availability mismatch (Kubernetes availability signal):}
\begin{minted}[fontsize=\small,breaklines]{yaml}
- alert: IXPApiGatewayUnavailable
  expr: kube_deployment_status_replicas_available{namespace="default",deployment="api-gateway"}
        < kube_deployment_spec_replicas{namespace="default",deployment="api-gateway"}
  for: 15s
\end{minted}
...
\textbf{Telemetry Kafka pipeline degradation (data pipeline signal):}
\begin{minted}[fontsize=\small,breaklines]{yaml}
...
Together, these examples show the layered rule strategy used in this project: platform availability, control-plane dependency health, and pipeline continuity.

This organization makes the alerting logic easier to read and easier to map back to operators' mental model of the system:

\begin{itemize}
    \item API Gateway alerts track request failures and Atomix access failures.
    \item Auction Runner alerts track auction progress and downstream dependency issues.
    \item Telemetry Processor alerts track Kafka consumption and Atomix write behavior.
\end{itemize}
...
Telegram routing is configured directly in the Helm values file used to deploy \texttt{kube-prometheus-stack}. In this project, the relevant configuration is stored under \texttt{observability/values.yaml} in the \texttt{alertmanager} section.
...
The observability components are deployed alongside the application services using Kubernetes manifests and Helm values. This includes the OpenTelemetry Collector, Prometheus, Alertmanager, and the supporting monitoring resources required for scraping and rule evaluation.
